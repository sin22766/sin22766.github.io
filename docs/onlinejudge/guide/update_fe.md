...
Copy the `dist` folder to a directory on the server, such as `/data/OnlineJudgeDeploy/data/backend/dist`, then modify `docker-compose.yml` in the `oj-backend` module Add a line `- /data/OnlineJudgeDeploy/data/backend/dist:/app/dist` in `volumes` (please modify the actual path before the colon), and then run `docker-compose up -d`.
